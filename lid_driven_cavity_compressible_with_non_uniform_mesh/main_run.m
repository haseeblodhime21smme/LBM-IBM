...
tau = (msh.chan_l*u_top)/(cs^2*Re) + 0.5;
omega = delt/tau;
vu = (tau - (delt/2))*(cs^2);
nu = vu*rho_avg;
NPOP = 9;
...
surf(msh.Y,msh.X,msh.node,'FaceColor','none','EdgeColor','black')
daspect([1 1 1])
view([0 0 1])
...
for t = 1:NSTEPS
    ...
    fprop = bc_calc_lid_comp(u_top,f,fprop,u,NX,NY,cs,rho,cvec,cvec_w);
end
toc
